            ...
            if ($booking_ok)
                ...
                elseif ($item->canceled == 1)
                  $item->book_link = '<span class="centered italic">' . JText::_('COM_SEMINARMAN_COURSE_CANCELED') . '</span>';              
            		...
            		$item->book_link = '<div class="button2-left"><div class="blank"><a href="' . JRoute::_('index.php?option=com_seminarman&view=courses&cid=' . $category->slug . '&id=' . $item->slug . '&Itemid=' . $Itemid . '&buchung=1#appform') . '">' . JText::_('COM_SEMINARMAN_BOOK_NOW') . '</a></div></div>';
            else
