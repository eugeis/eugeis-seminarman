...
class seminarmanControllerApplication extends seminarmanController
{
	function save()
	{
		...
		if ($params->get('current_capacity') > 0 && $courseRows->capacity - (int)$applicationModel->getCurrentBookings() < $post['attendees'])
...
$priceg5_reg = $priceg5['reg_group'];
		...
		if ((($params->get('invoice_generate') == 1) && ($post['price_per_attendee'] > 0)) || ($post['booking_price'][0] == 66)) {
		...
		if (($params->get('trigger_virtuemart') == 1) && (!is_null($vmlink))) {
		    ...
		    // send confimation mail with invoice to applicant
		    if ($params->get('invoice_generate') == 1 && $params->get('invoice_attach') == 1 && ($post['price_per_attendee'] > 0))
